...
/**
  ...
  */
void Emm_V5_Clear_cnt(uint8_t addr)
{
   uint8_t cmd[16] = {0};
   cmd[0] = addr;
   cmd[1] = 0x0A;
...
/**
  ...
  * @retval   地址 + 功能码 + 校验字节
  */
void Emm_V5_Read_cnt(uint8_t addr)
{
   uint8_t cmd[16] = {0};
   cmd[0] = addr;
   cmd[1] = 0x32;
  ...
  HAL_UART_Transmit(&huart3,cmd,3,HAL_MAX_DELAY);   
}
...
void Emm_V5_Origin_Modify_Params(uint8_t addr, uint8_t svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, uint8_t potF)
{
  ...
  // 发送命令
  HAL_UART_Transmit(&huart3,cmd,20,HAL_MAX_DELAY);
}
